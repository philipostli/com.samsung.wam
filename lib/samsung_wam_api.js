...
module.exports = class SamsungWAMApi {
  ...
  async getFunc() {
    const funcStatus = await this.get('<name>GetFunc</name>');
  ...
  async setFunc(func) {
    ...
    if (current !== func) {
      ...
      await this.get(`<name>SetFunc</name><p type="str" name="function" val="${func}"/>`);
      return true;
    }
  }

  async getVolume() {
    const volumeStatus = await this.get('<name>GetVolume</name>');
    return volumeStatus.UIC.response[0].volume ? parseInt(volumeStatus.UIC.response[0].volume[0]) : 0;
  }

  async setVolume(volume) {
    await this.get(`<name>SetVolume</name><p type="dec" name="volume" val="${volume}"/>`);
  }
  ...
  async getMuted() {
    const muteStatus = await this.get('<name>GetMute</name>');
